...
class Executor
  def initialize()
    @input = ARGV[0]
    @canvas_instance = nil
    @output = File.join(File.expand_path(File.dirname(__FILE__)), 'output.txt')
  end
  ...
  def process_file
    File.open(@input).each do |line|
      case line
      when /^C\s(\d+)\s(\d+)$/
        @canvas_instance = Canvas.new($1.to_i, $2.to_i)
        @canvas_instance.write_file(@output)
      when /^L\s(\d+)\s(\d+)\s(\d+)\s(\d+)$/
        @canvas_instance.line($1.to_i, $2.to_i, $3.to_i, $4.to_i)
        @canvas_instance.write_file(@output)
      when /^R\s(\d+)\s(\d+)\s(\d+)\s(\d+)$/
        @canvas_instance.rectangle($1.to_i, $2.to_i, $3.to_i, $4.to_i)
        @canvas_instance.write_file(@output)
      when /^B\s(\d+)\s(\d+)\s(.)$/
        @canvas_instance.bucket_fill($1.to_i, $2.to_i, $3.to_i)
        @canvas_instance.write_file(@output)
      else
